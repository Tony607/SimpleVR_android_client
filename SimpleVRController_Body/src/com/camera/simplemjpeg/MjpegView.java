...
public class MjpegView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private final static int VR_LEFT = 16;
    private final static int VR_RIGHT = 32;
    ...
    public class MjpegViewThread extends Thread {
        ...
        private Rect destRectVR(int bmw, int bmh, int leftorright) {
            if (leftorright == MjpegView.VR_LEFT) {
            	return new Rect(0, 0, dispWidth/2, dispHeight);
            }
            if (leftorright == MjpegView.VR_RIGHT) {
            	return new Rect(dispWidth/2, 0, dispWidth, dispHeight);
            }
            return null;
        }
        ...
        public void run() {
            ...
            while (mRun) {
                ...
                Rect destRectLeft=null;
                Rect destRectRight=null;
                Canvas c = null;
                ...
                if(surfaceDone) {   
                	try {
                		...
                		if(ret == -1)
                		{
                			((CameraActivity)saved_context).setImageError();
                			return;
                		}
                		
                		destRectLeft = destRectVR(bmp.getWidth(),bmp.getHeight(),VR_LEFT);
                		destRectRight = destRectVR(bmp.getWidth(),bmp.getHeight(),VR_RIGHT);
                		
                        c = mSurfaceHolder.lockCanvas();
                        synchronized (mSurfaceHolder) {

                               	c.drawBitmap(bmp, null, destRectLeft, p);
                               	c.drawBitmap(bmp, null, destRectRight, p);
                                if(showFps) {
                                    p.setXfermode(mode);
                                    if(ovl != null) {

                                    	// false indentation to fix forum layout 	                                	 
                                    	height = ((ovlPos & 1) == 1) ? destRectLeft.top : destRectLeft.bottom-ovl.getHeight();
                                    	width  = ((ovlPos & 8) == 8) ? destRectLeft.left : destRectLeft.right -ovl.getWidth();
